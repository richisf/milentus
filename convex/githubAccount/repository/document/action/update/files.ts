...
export const document = internalAction({
  ...
  handler: async (ctx, args): Promise<{
    ...
    try {
      console.log(`📁 Starting file-based processing for repository: ${args.repositoryId}`);

      const createFilesResult = await ctx.runAction(internal.githubAccount.repository.document.files.action.create.files, {
