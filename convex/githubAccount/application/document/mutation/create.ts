...
export const document = internalMutation({
  ...
  handler: async (ctx, args): Promise<Id<"document">> => {
    ...
    const application = await ctx.runQuery(internal.githubAccount.application.query.by_id.application, {
    ...
    const existingDocument = await ctx.runQuery(internal.githubAccount.application.document.query.by_application.document, {
      applicationId: args.applicationId,
    });
