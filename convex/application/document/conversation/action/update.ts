...
export const conversation = internalAction({
  ...
  handler: async (ctx, args): Promise<{
    ...
    const conversation = await ctx.runQuery(
      internal.application.document.conversation.query.by_document.by_id,
    ...
    if (args.clear) {
      ...
      const conversationHistory = await ctx.runQuery(
        internal.application.document.conversation.message.query.by_conversation.messagesInternal,
        { conversationId: args.conversationId }
      );
      ...
      for (const message of conversationHistory) {
        await ctx.runMutation(
          internal.application.document.conversation.message.mutation.delete.message,
    ...
    const rawMessageResult = await ctx.runAction(
      internal.application.document.conversation.message.action.create.message,
      {
    ...
    const saveResult = await ctx.runMutation(
      internal.application.document.conversation.mutation.update.conversation,
      {
        conversationId: args.conversationId,
    ...
    if (messageResult.shouldUpdateDocument && messageResult.nodes && messageResult.nodes.length > 0) {
      ...
      await ctx.runMutation(
        internal.application.document.mutation.update.document,
        {
          documentId: conversation.documentId,
